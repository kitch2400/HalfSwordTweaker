...
public class IniFile
{
    ...
    /// <summary>
    /// Gets the sections dictionary (public for testing).
    /// </summary>
    public Dictionary<string, Dictionary<string, string>> Sections => _sections;
    ...
    /// <summary>
    /// Writes the INI file to disk, preserving existing sections not in memory.
    /// </summary>
    ...
    public bool Write()
    {
        try
        {
            ...
            // Build output: preserve existing sections not in _sections
            var outputLines = new List<string>();
            var sectionsInMemory = new HashSet<string>(_sections.Keys, StringComparer.OrdinalIgnoreCase);
            ...
            while (i < existingLines.Count)
            {
                ...
                if (trimmedLine.StartsWith("[") && trimmedLine.EndsWith("]"))
                {
                    ...
                    if (sectionsInMemory.Contains(sectionName))
                    {
                        // Write from memory instead
                        outputLines.Add(trimmedLine);
                        foreach (var setting in _sections[sectionName])
                        {
                            outputLines.Add($"{setting.Key}={setting.Value}");
                        }
                        ...
                        while (i < existingLines.Count)
                        {
                            var nextLine = existingLines[i].Trim();
