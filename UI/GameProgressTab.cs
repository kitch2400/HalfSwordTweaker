...
public class CharacterWeightControl : Panel
{
    ...
    public double Value
    {
        ...
        set
        {
            _value = Math.Clamp(value, (double)_numericUpDown.Minimum, (double)_numericUpDown.Maximum);
            _numericUpDown.Value = (decimal)_value;
            UpdateKgLabel();
        }
    }
    ...
    public CharacterWeightControl(string name, string description, double minValue, double maxValue, double defaultValue)
    {
        ...
        // Initialize backing field to match the NumericUpDown value
        _value = (double)_numericUpDown.Value;
        ...
        foreach (var kvp in properties)
        {
            if (_settingControls.TryGetValue(kvp.Key, out var control))
            {
                    ...
                    switch (control)
                    {
                        ...
                        case CharacterWeightControl weightControl:
                            if (kvp.Value is double weightValue)
                                weightControl.Value = weightValue;
                            break;
                ...
                catch (Exception ex)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"Error setting value for {kvp.Key}: {ex.Message}");
                }
            }
        }
