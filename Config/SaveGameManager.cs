...
public class SaveGameManager
{
    ...
    {
        return ReadSettings(_settingsSavePath);
    }

    /// <summary>
    /// Reads settings from a specific file path (for testing).
    /// </summary>
    /// <param name="filePath">The path to the Settings.sav file.</param>
    /// <returns>A dictionary of setting names and their values.</returns>
    public Dictionary<string, double> ReadSettings(string filePath)
    {
        Console.Error.WriteLine($"[SaveGameManager] Reading from: {filePath}");
        
        if (!File.Exists(filePath))
        {
            Console.Error.WriteLine($"[SaveGameManager] {filePath} does not exist!");
    ...
    {
        return WriteSettings(_settingsSavePath, settings);
    }

    /// <summary>
    /// Writes settings to a specific file path (for testing).
    /// </summary>
    /// <param name="filePath">The path to write the Settings.sav file.</param>
    /// <param name="settings">The settings to write.</param>
    /// <returns>True if successful; otherwise, false.</returns>
    public bool WriteSettings(string filePath, Dictionary<string, double> settings)
    {
        Console.Error.WriteLine($"[SaveGameManager] Writing to: {filePath}");
        Console.Error.WriteLine($"[SaveGameManager] Settings to write: {settings.Count}");
        
        try
        {
            // Create backup first (only for production path)
            if (filePath == _settingsSavePath)
            {
                CreateBackup();
            }
            ...
            else
            {
                ...
                Console.Error.WriteLine($"[SaveGameManager] {filePath} does not exist, creating from template");
                try
                {
                    data = (byte[])SettingsTemplate.Data.Clone();
                    Console.Error.WriteLine($"[SaveGameManager] Created new {filePath} from template ({data.Length} bytes)");
                }
                catch (Exception ex)
                {
            ...
            File.WriteAllBytes(filePath, updatedData);
            ...
            var verifyBytes = File.ReadAllBytes(filePath);
