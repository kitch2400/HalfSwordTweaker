...
public class MainForm : Form
{
    ...
    public MainForm()
    {
        ...
        // Load cache synchronously (fast, just reads local file)
        DescriptionFetcher.LoadCache();

        InitializeComponent();
        LoadSettings();
        
        // Start tooltip initialization asynchronously WITHOUT blocking
        _ = InitializeTooltipsAsync();
    ...
    private void UpdateTabAsterisks()
    {
        ...
        // Engine Tweaks tab
        if (_engineTweaksTab != null)
        {
            _engineTweaksTab.Text = _engineTweaksTab.HasChanges 
                ? "Engine Tweaks *" 
                : "Engine Tweaks";
        }
    ...
    private async Task InitializeTooltipsAsync()
    {
        ...
        catch (Exception ex)
        {
            ...
            Console.WriteLine($"Tooltip initialization failed: {ex.Message}");
        }
        finally
        {
    ...
    private EngineTweaksTab? _engineTweaksTab;

    private void InitializeTabs()
    {
        // Add Display & Resolution tab first
        var displayTab = new CategoryTab("Display & Resolution");
        foreach (var setting in SettingsRegistry.Tabs["Display & Resolution"])
        {
            displayTab.AddSetting(setting);
        }
        _tabControl.TabPages.Add(displayTab);

        // Add Engine Tweaks tab second
        _engineTweaksTab = new EngineTweaksTab(_tooltip);
        _tabControl.TabPages.Add(_engineTweaksTab);

        // Add Scalability Groups tab third
        var scalabilityTab = new CategoryTab("Scalability Groups");
        foreach (var setting in SettingsRegistry.Tabs["Scalability Groups"])
        {
            scalabilityTab.AddSetting(setting);
        }
        _tabControl.TabPages.Add(scalabilityTab);
    ...
    private void ApplySettings()
    {
        try
        {
            ...
            // Apply Engine Tweaks settings (only if there are changes)
            _engineTweaksTab?.ApplyChanges();
    ...
    private void MainForm_FormClosing(object? sender, FormClosingEventArgs e)
    {
        ...
        if (_engineTweaksTab != null && _engineTweaksTab.HasChanges)
        {
            unsavedTabs.Add("Engine Tweaks");
        }
    ...
    private void RefreshSettings()
    {
        if (_configManager.ReadAll())
        {
            ...
            _engineTweaksTab?.LoadCurrentSettings();
